    ...
    public function assignmentSubmitted(array $data): void {
        try {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                $operatorName = trim((string)$sheet->getCell("C$row")->getValue()); // example column for operator
                $vehicleNumber = trim((string)$sheet->getCell("B$row")->getValue()); // example column for vehicle
                $startDateTime = trim((string)$sheet->getCell("D$row")->getValue()); // example column for start datetime
            ...
            if (!$matchRow) {
                error_log("[AssignmentExporter] No matching row found for operator {$data['operator_name']}, vehicle {$data['vehicle_number']}, start {$data['start_date_time']}");
                return;
            }

            // Map DB fields to Excel columns
            $columns = [
                'actual_drop_time' => 'E',
                'actual_end_time' => 'F',
                'total_hrs' => 'G',
            ...
            foreach ($columns as $field => $col) {
                ...
                if ($currentValue !== $newValue && $newValue !== '') {
                    ...
                    error_log("[AssignmentExporter] Updated $field in row $matchRow to $newValue");
                }
            }
            ...
            error_log("[AssignmentExporter] Error updating Excel: " . $e->getMessage());
        }
    }
};
